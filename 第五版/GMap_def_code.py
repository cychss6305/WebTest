...
def get_url(search_url,query,id):
    try: #錯誤訊息 : https://dotblogs.com.tw/caubekimo/2018/09/17/145733
        ...
        for i in place_id: #1 、 5 筆
            ...
            if url != '': #Gmap有找到這家公司的url
                try: #Gmap有找到這家公司的電話
                    ...
                    if request_company == company_name:
                        phonecheck = 1
                    else:
                        phonecheck = -1
        ...
        if len(url_list) > 0 : #放x_list資料        
            ...
            if not folder:
                    os.makedirs(path)
            json_name = path + request_number+'.json'
...
def input_error(error):
    for i in map_json.keys():
        ...
        if i == 'Status':
            map_json[i].append(error)
def cut_request_url(url,name):
    strname = name
    left = url.find(strname)
    new_url = url[left:]
    strlen = len(strname)
    right = (new_url).find('&')
    if right == -1:
        right = len(new_url)
    return new_url[strlen:right]

#@app.route('/',methods=['GET'])
def result_dict(url):
    global map_json,request_number,request_search_phone,request_company,request_home,errMsg
    
    #request_number = request.args.get('App_Case_No')
    #request_search_phone = request.args.get('Comp_Tel')
    #request_company = request.args.get('query')
    #request_home = request.args.get('home')
    
    request_number = cut_request_url(url,'App_Case_No=')
    request_search_phone = cut_request_url(url,'Comp_Tel=')
    request_company = cut_request_url(url,'query=')
    request_home = cut_request_url(url,'home=')
