...
def get_url(search_url,query,id):
    try: #錯誤訊息 : https://dotblogs.com.tw/caubekimo/2018/09/17/145733
        ...
        if id == 'getphone': 
            ...
            else: #找不到 or 使用電話/公司名稱搜尋後，發現有多筆，則回傳空值即可
                return url
        tEnd = time.time()
        ...
        for i in place_id: #1 、 5 筆
            ...
            if url != '': #Gmap有找到這家公司的url
                ...
                ##################################################################################API4 : 車距、車時
                tStart = time.time()#計時開始
                temp_dist_time,home_Ename = get_googlemaps_distance(details_json["result"]["formatted_address"],home_name) #Ans,home_Ename(用英文名稱找會更好，不易出錯)
...
def result_dict():
    ...
    request_number = request.values['App_Case_No'].replace(" ", "")
    request_search_phone = request.values['Comp_Tel'].replace(" ", "")
    request_company = request.values['query'].replace(" ", "")
    request_home = request.values['home'].replace(" ", "")
    
    
    map_json =  {'Status':[],'App_Case_No':[],'Comp_Tel':[],
             'Comp_Nme':[],'GMap_Tel_Check':[],'GMap_URL':[],
             'GMap_Site':[],'GMap_Add':[],'GMap_Tel':[],
             'GMap_Distance':[],"GMap_Score":[]} 
             #'GMap_Catg':[],'GMap_Near_Eat':[],'GMap_Nearr_Comp':[],'GMap_Near_Edu':[]} 
